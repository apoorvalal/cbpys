...
class ATTWeightEstimator(ABC):
    ...
    @abstractmethod
    def fit(self, X_treat: np.ndarray, X_control: np.ndarray, y_treat: np.ndarray, y_control: np.ndarray, **kwargs):
        """
    ...
    def estimate_att(self) -> float:
        ...
        if self.weights is None or self.y_treat_mean is None or self.y_control is None or self.sum_weights is None:
             # Check if DML was intended and failed, leading to these being None
            if hasattr(self, '_dml_att_estimate') and self._dml_att_estimate is not None:
                ...
                pass # Let subclass logic proceed if it has _dml_att_estimate
                 ...
                 print("Warning: DML was used, but attributes for non-DML ATT calculation are missing and no DML estimate available.")
                 return np.nan
            raise RuntimeError("Estimator has not been fitted correctly for non-DML ATT calculation.")

        if abs(self.sum_weights) < 1e-9:
            import warnings
            warnings.warn("Sum of weights is close to zero. ATT estimate might be unstable or NaN.", UserWarning)
            return np.nan if self.sum_weights == 0 else self.y_treat_mean

        weighted_control_outcome = np.sum(self.weights * self.y_control) / self.sum_weights
...
class PenalizedFormEstimator(ATTWeightEstimator):
    ...
    """
    def fit(self, X_treat: np.ndarray, X_control: np.ndarray, y_treat: np.ndarray, y_control: np.ndarray, delta: float = 1.0, **kwargs):
        """
        ...
        phi_q_mean = np.mean(X_treat, axis=0) # E_1[phi(X)], target mean feature vector
        ...
        imbalance_vector = (w_var.T @ phi_p) / n_control - phi_q_mean # This is a row vector (1, k_features)
        imbalance = cp.sum_squares(imbalance_vector)
        penalty = cp.sum_squares(w_var) / n_control # Sum of squares of elements of w_var, divided by n_control
        ...
        if w_var.value is None:
            raise RuntimeError(f"CVXPY optimization failed for PenalizedFormEstimator. Problem status: {problem.status}")
        ...
        if abs(self.sum_weights) < 1e-9: # Check for very small sum of weights
            print("Warning: Sum of weights is close to zero in PenalizedFormEstimator.")
...
class ConstrainedFormEstimator(ATTWeightEstimator):
    ...
    where w are the weights for control units.
    """
    def fit(self, X_treat: np.ndarray, X_control: np.ndarray, y_treat: np.ndarray, y_control: np.ndarray, delta_constraint: float = 0.1, **kwargs):
        """
        ...
        phi_q_sum = np.sum(X_treat, axis=0) # Target sum of features for control group using weights
        ...
        objective = cp.Minimize(cp.sum_squares(w_var)) # Minimize L2 norm of weights: ||w||^2
        ...
        problem.solve(solver=cp.ECOS) # Added a specific solver, can be SCS as well

        if w_var.value is None or problem.status in [cp.INFEASIBLE, cp.UNBOUNDED, cp.INFEASIBLE_INACCURATE]:
            # This can happen if delta_constraint is too small (infeasible)
            print(f"Warning: CVXPY optimization failed or found no solution for ConstrainedFormEstimator. "
        ...
        if abs(self.sum_weights) < 1e-9 and w_var.value is not None:
             print("Warning: Sum of weights is close to zero in ConstrainedFormEstimator.")


class RieszRegressionFormEstimator(ATTWeightEstimator):
    """
    Estimates ATT by solving for the Riesz representer `theta` in feature space,
    then constructing weights for control units as `w = X_control @ theta`.
    ...
    def fit(self, X_treat: np.ndarray, X_control: np.ndarray, y_treat: np.ndarray, y_control: np.ndarray, delta: float = 1.0, **kwargs):
        ...
        try:
            ...
            print(f"Warning: numpy.linalg.solve failed for RieszRegressionFormEstimator. Using pseudo-inverse. Delta: {delta}")
            theta_value = np.linalg.pinv(A_matrix) @ phi_q_mean

        self.weights = phi_p @ theta_value
        self.y_treat_mean = np.mean(y_treat)
        self.y_control = y_control
        self.sum_weights = np.sum(self.weights) # These weights are not constrained to sum to anything specific by default.
                                              # Normalization happens in estimate_att.
        if abs(self.sum_weights) < 1e-9:
            print("Warning: Sum of weights is close to zero in RieszRegressionFormEstimator.")

from sklearn.metrics.pairwise import rbf_kernel, polynomial_kernel, linear_kernel

class KernelRieszATT(ATTWeightEstimator):
    """
    Estimates ATT using Riesz representer weights learned in a kernel-induced RKHS.
    The weights aim to balance the mean embedding of treated and control covariates in the RKHS.
    Features include:
    ...
    def __init__(self,
        ...
        self.reg_param_grid = reg_param_grid if reg_param_grid is not None else [default_reg_param]

        self.cv_folds = cv_folds
        self.cv_scoring_metric = cv_scoring_metric

        self.default_kernel_args = default_kernel_args if default_kernel_args is not None else {}
        ...
        super().use_dml = self.use_dml # Update base class attribute if needed/exists

        if kwargs:
            print(f"Warning: KernelRieszATT received unused keyword arguments: {list(kwargs.keys())}")
    ...
    def _calculate_mmd_squared(self, X_treat_val, X_control_val, weights_c_val, current_kernel_args):
        """
        ...
        if n_t_val == 0 or n_c_val == 0:
            return np.inf # Should not happen with proper CV splits if data is sufficient

        sum_w = np.sum(weights_c_val)
        if abs(sum_w) < 1e-9:
            return np.inf

        K_tt_val = self.kernel_func(X_treat_val, X_treat_val, **current_kernel_args)
        K_cc_val = self.kernel_func(X_control_val, X_control_val, **current_kernel_args)
        K_tc_val = self.kernel_func(X_treat_val, X_control_val, **current_kernel_args) # (n_t_val, n_c_val)
    ...
    def _fit_single_config(self, X_treat_fit, X_control_fit, current_kernel_args, current_reg_param):
        """
        ...
        if n_control_fit == 0 or n_treat_fit == 0: # Should be handled by CV split checks
            return np.array([]) # Or raise error

        K_cc_fit = self.kernel_func(X_control_fit, X_control_fit, **current_kernel_args)
        K_xc_xt_fit = self.kernel_func(X_control_fit, X_treat_fit, **current_kernel_args)
        k_target_vector_fit = np.mean(K_xc_xt_fit, axis=1)

        A_matrix_fit = K_cc_fit + n_control_fit * current_reg_param * np.eye(n_control_fit)

        try:
            weights = np.linalg.solve(A_matrix_fit, k_target_vector_fit)
        except np.linalg.LinAlgError:
            # Fallback to pseudo-inverse if singular
            print(f"Warning (_fit_single_config): Using pseudo-inverse. Reg: {current_reg_param}, Kernel: {current_kernel_args}")
            weights = np.linalg.pinv(A_matrix_fit) @ k_target_vector_fit
        return weights

    def fit(self, X_treat: np.ndarray, X_control: np.ndarray, y_treat: np.ndarray, y_control: np.ndarray, delta: float = None, **kwargs_fit):
        """
        ...
        """
        from sklearn.model_selection import KFold, ParameterGrid # Keep import here for clarity on usage
        ...
        if n_control_orig == 0 or n_treat_orig == 0:
            raise ValueError("Treated and control groups must not be empty for fitting.")
        ...
        if self.kernel_param_grid: # If grid for kernel params exists
            ...
            if len(param_list) > 1 or (len(param_list)==1 and param_list[0] != self.default_kernel_args):
                run_cv = True
        if len(self.reg_param_grid) > 1:
             run_cv = True
        if delta is not None: # User override for reg_param, CV for reg_param is skipped.
            current_reg_params_to_search = [delta]
            if len(self.reg_param_grid) >1 : # if delta overrides a grid search for reg_param
                 run_cv = bool(self.kernel_param_grid) # CV only if kernel_param_grid is active
        else:
            current_reg_params_to_search = self.reg_param_grid

        if not run_cv and not self.kernel_param_grid and len(current_reg_params_to_search) == 1:
            ...
            print("Warning: cv_folds < 2, skipping CV. Using default parameters.")
            self.best_kernel_args_ = self.default_kernel_args
            self.best_reg_param_ = current_reg_params_to_search[0] # Use first from list if grid was given
        ...
        if run_cv:
            ...
            kernel_configs = list(ParameterGrid(self.kernel_param_grid)) if self.kernel_param_grid else [self.default_kernel_args]
            ...
            kf_treat = KFold(n_splits=self.cv_folds, shuffle=True, random_state=42)
            kf_control = KFold(n_splits=self.cv_folds, shuffle=True, random_state=43) # Different seed
            ...
            for reg_param_val in current_reg_params_to_search:
                for kernel_args_val in kernel_configs:
                    ...
                    for i in range(self.cv_folds):
                        ...
                        if X_t_val_fold.shape[0] == 0 or X_c_val_fold.shape[0] == 0:
                            ...
                            print(f"Warning: Empty validation fold for T({X_t_val_fold.shape[0]}) or C({X_c_val_fold.shape[0]}). Skipping fold.")
                        ...
                        weights_val_fold = self._fit_single_config(
                            X_t_val_fold, X_c_val_fold,
                            kernel_args_val, reg_param_val
                        )

                        if weights_val_fold.size == 0 or np.sum(np.isnan(weights_val_fold)) > 0 : # check if _fit_single_config failed
                            fold_scores.append(np.inf)
                            continue

                        if self.cv_scoring_metric == 'mmd':
                            score = self._calculate_mmd_squared(
                                X_t_val_fold, X_c_val_fold, weights_val_fold, kernel_args_val
                            )
                            fold_scores.append(score)
                        else:
                            raise ValueError(f"Unsupported cv_scoring_metric: {self.cv_scoring_metric}")

                    current_mean_score = np.mean([s for s in fold_scores if np.isfinite(s)]) if any(np.isfinite(s) for s in fold_scores) else np.inf
            ...
            if np.isinf(best_score):
                print("Warning: CV did not find valid parameters. Falling back to defaults.")
                self.best_kernel_args_ = self.default_kernel_args
                self.best_reg_param_ = self.default_reg_param if delta is None else delta
        ...
        if self.use_dml:
            if self.dml_folds < 2:
                print("Warning: dml_folds < 2, DML cannot be performed. Fitting on full data instead.")
        ...
        if self.use_dml:
            ...
            if min_samples_t < 1 or min_samples_c < 1 : # Crude check, KFold might handle n_splits > n_samples better
                 print(f"Warning: dml_folds ({self.dml_folds}) may be too large for sample sizes (T:{n_treat_orig}, C:{n_control_orig}). Reducing dml_folds.")
                 self.dml_folds = min(n_treat_orig, n_control_orig, self.dml_folds)
                 if self.dml_folds < 2:
                     print("Fallback: Not enough data for DML. Fitting on full data.")
                     self.use_dml = False # Fallback if still too few folds

        if self.use_dml:
            print(f"Info: Performing DML with {self.dml_folds} folds using reg={final_reg_param}, kernel={final_kernel_args}")
            kf_treat_dml = KFold(n_splits=self.dml_folds, shuffle=True, random_state=123)
            kf_control_dml = KFold(n_splits=self.dml_folds, shuffle=True, random_state=124)
            ...
            for i in range(self.dml_folds):
                ...
                if X_t_k.shape[0] == 0 or X_c_k.shape[0] == 0:
                    print(f"Warning (DML fold {i}): Empty data split for T({X_t_k.shape[0]}) or C({X_c_k.shape[0]}). Skipping fold.")
                    continue

                weights_k = self._fit_single_config(
                    X_t_k, X_c_k,
                    final_kernel_args, final_reg_param
                )

                if weights_k.size == 0 or np.sum(np.isnan(weights_k)) > 0:
                    print(f"Warning (DML fold {i}): Weight computation failed. Skipping fold.")
                    continue

                sum_weights_k = np.sum(weights_k)
                if abs(sum_weights_k) < 1e-9:
                    print(f"Warning (DML fold {i}): Sum of weights is zero. Skipping ATT for this fold.")
                    # This fold won't contribute to the mean ATT. Or could be np.nan.
                    continue

                att_k = np.mean(y_t_k) - np.sum(weights_k * y_c_k) / sum_weights_k
                fold_att_estimates.append(att_k)

            if not fold_att_estimates: # All folds failed
                print("Error: DML estimation failed for all folds. No ATT estimate produced.")
            ...
            self.weights = None # Indicate that single overall weights are not applicable.
            ...
            if self.weights.size == 0:
                 raise RuntimeError(f"Final weight computation failed with params: {final_kernel_args}, reg: {final_reg_param}")
            ...
            if abs(self.sum_weights) < 1e-9:
                print(f"Warning: Sum of weights is close to zero in KernelRieszATT (final fit). Sum: {self.sum_weights}, Reg: {final_reg_param}, Kernel: {final_kernel_args}")
    ...
    def estimate_att(self) -> float:
        ...
        if self.weights is None or self.y_treat_mean is None or self.y_control is None or self.sum_weights is None:
            ...
            if self.use_dml : # and DML failed to produce an estimate
                 print("Error: DML was used but failed to produce an estimate, and no fallback weights available.")
                 return np.nan
            raise RuntimeError("Estimator has not been fitted correctly (non-DML path).")

        if abs(self.sum_weights) < 1e-9 : # Check for sum_weights being effectively zero
            print("Warning (estimate_att): Sum of weights is close to zero. ATT estimate might be unstable or NaN.")
            # Depending on y_treat_mean, this could be y_treat_mean or NaN.
            # If sum_weights is zero, weighted_control_outcome is undefined.
            return np.nan if self.sum_weights == 0 else self.y_treat_mean # if sum_weights is tiny but non-zero.
    ...
    def fit(self, X_treat: np.ndarray, X_control: np.ndarray, y_treat: np.ndarray, y_control: np.ndarray, delta: float = None):
        ...
        pass # Placeholder for diff utility
        ...
        sum_w = np.sum(weights_c_val)
        if abs(sum_w) < 1e-9: # Avoid division by zero if weights sum to nothing
            return np.inf
    ...
    def _fit_single_config(self, X_treat, X_control, current_kernel_args, current_reg_param, n_treat_fit, n_control_fit):
        ...
        K_xc_xt_fit = self.kernel_func(X_control, X_treat, **current_kernel_args)
        k_target_vector_fit = np.mean(K_xc_xt_fit, axis=1)
    ...
    def fit(self, X_treat: np.ndarray, X_control: np.ndarray, y_treat: np.ndarray, y_control: np.ndarray, delta: float = None):
        ...
        try:
            ...
            print(f"Warning: numpy.linalg.solve failed for KernelRieszATT. Using pseudo-inverse. Reg param: {current_reg_param}")
        ...
        if abs(self.sum_weights) < 1e-9:
            print(f"Warning: Sum of weights is close to zero in KernelRieszATT. Sum: {self.sum_weights}, Reg param: {current_reg_param}")
...
# Example of how kang_schafer might be used (will be in the other file)
# class kang_schafer: ... (from user prompt)
# def expand_polynomial_features: ... (from user prompt)
# def expand_rbf_features: ... (from user prompt)

# The main simulation logic will be adapted to use these classes.
# For now, this file `att_estimators.py` is complete.
